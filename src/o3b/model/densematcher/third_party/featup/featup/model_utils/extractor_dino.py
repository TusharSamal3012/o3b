...
class ViTExtractor(nn.Module):
    ...
    def resize_and_normalize(self, image_tensor, size):
        """
        arg: float tensor of shape [bs, C, H, W]
        """
        image_tensor_resized = torch.nn.functional.interpolate(image_tensor, size=(size, size), mode='bilinear', align_corners=False)
        prep = transforms.Compose([
            transforms.Normalize(mean=self.mean, std=self.std)
        ])
        prep_img = prep(image_tensor_resized)
        return prep_img
    ...
    def _log_bin(self, x: torch.Tensor, hierarchy: int = 2) -> torch.Tensor:
        ...
        bin_x = torch.zeros((B, sub_desc_dim * num_bins, self.num_patches[0], self.num_patches[1])).to(x.device)
