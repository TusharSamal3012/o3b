...
class JBUFeatUp(pl.LightningModule):
    def __init__(self,
        ...
        self.model, self.patch_size, self.dim = get_featurizer(model_type, final_size, num_classes=1000, rot_inv=self.rot_inv)
        self.model.mem_eff = mem_eff
        for p in self.model.parameters():
            p.requires_grad = False
        
        self.model = torch.nn.Sequential(self.model)
        if use_unitnorm:
            self.model.append(UnitNorm(self.dim))
        if use_channelnorm:
            self.model.append(ChannelNorm(self.dim))
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(self.n_jitters):
            ...
            if os.environ.get("VERBOSE", False):
                print("featup desc norm:", hr_feats.norm(dim=1).min(), hr_feats.norm(dim=1).max(), hr_feats.norm(dim=1).mean())
        ...
        if self.global_step % 1000 == 0:
            self.trainer.save_checkpoint(self.chkpt_dir[:-5] + '/periodic/' + os.path.basename(self.chkpt_dir[:-5]) + f'_{self.global_step}.ckpt')
...
def my_app(cfg: DictConfig) -> None:
    ...
    cfg.output_root = f"{cfg.output_root}_imsize={cfg.img_size}_steps={cfg.train_steps}_channelnorm={cfg.channelnorm}_unitnorm={cfg.unitnorm}_rotinv={cfg.rot_inv}"
    print(cfg.output_root)
    seed_everything(seed=0, workers=True) # this seeds every worker differently. Workers are spawned when dataloader starts being iterated

    load_size = cfg.img_size
    assert load_size % 64 == 0, "image size needs to be multiple of 64 for sd"
    ...
    model = JBUFeatUp(
        ...
        use_channelnorm=cfg.channelnorm,
        use_unitnorm=cfg.unitnorm,
        max_pad=cfg.max_pad,
        max_zoom=cfg.max_zoom,
        kernel_size=16,
        final_size=cfg.img_size // 16,
        lr=cfg.lr,
    ...
    # As per https://pytorch-lightning.readthedocs.io/en/0.9.0/multi_gpu.html#:~:text=DistributedSampler%20for%20multi%2Dnode%20or,appropriate%20part%20of%20your%20data.&text=Lightning%20adds%20the%20correct%20samplers,need%20to%20explicitly%20add%20samplers.&text=For%20iterable%20datasets%2C%20we%20don't%20do%20this%20automatically.
    # pytorch lightning trainer replaces samplers with distributedsamplers under the hood, so you dont need to worrry about it
    loader = DataLoader(
        dataset, cfg.batch_size, shuffle=True, num_workers=cfg.num_workers)
    val_loader = DataLoader(
        SingleImageDataset(0, dataset, 1), 1, shuffle=False, num_workers=1)
