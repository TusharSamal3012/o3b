...
class ImageNetSubset(VisionDataset):
    ...
    def __init__(self,
        ...
        if subset is not None:
            ...
            print("start loading npy")
            self.entries = np.load(join(root, "ImageNet_extra", f"entries-{split.upper()}.npy"))
            self.targets = None
            if len(self.entries) == 0:
                raise RuntimeError('dataset is empty')
            print("finished loading npy")
            np.random.shuffle(self.entries) # this is deterministic
            print("zeroth entry", self.entries[0]) # this should be the same across processes
    ...
    def __getitem__(self, index):
        ...
        class_id, file_idx = entry["class_id"], entry["actual_index"]
        image_path = join(self.root, self.split, class_id, f"{class_id}_{file_idx}.JPEG")
...
if __name__ == "__main__":
    dataset = ImageNetSubset("/mnt/disks/tepan_datasets", "train", include_labels=False)
    print(np.asarray(dataset[0]['img']).shape)
