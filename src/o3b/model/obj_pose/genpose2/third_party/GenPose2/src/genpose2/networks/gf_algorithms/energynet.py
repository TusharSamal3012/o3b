...
from genpose2.networks.gf_algorithms.samplers import cond_ode_likelihood, cond_ode_sampler, cond_pc_sampler
from genpose2.networks.gf_algorithms.scorenet import GaussianFourierProjection
from genpose2.networks.pts_encoder.pointnets2 import Pointnet2ClsMSG
from genpose2.networks.pts_encoder.pointnets import PointNetfeat
from genpose2.utils.genpose_utils import get_pose_dim
...
class PoseEnergyNet(nn.Module):
    ...
    def output_zero_initial(self):
        if self.regression_head == 'RT':
            ...
            zero_module(self.fusion_tail_angle[-1])
            zero_module(self.fusion_tail_vec[-1])
            zero_module(self.fusion_tail_trans[-1])    
    ...
    def get_energy(self, pts_feat, rgb_feat, sampled_pose, t, decoupled_rt=True):
        ...
        if rgb_feat is not None:
            total_feat = torch.cat([total_feat, rgb_feat], dim=-1)
    ...
    def forward(self, data, return_item='score'):
        ...
        rgb_feat = data['rgb_feat'] if self.dino_dim else None
        sampled_pose = data['sampled_pose']
        t = data['t']

        if return_item == 'energy':
            energy = self.get_energy(pts_feat, rgb_feat, sampled_pose, t)
            return energy
        
        with TemporaryGrad():
            inp_variable_sampled_pose = Variable(sampled_pose, requires_grad=True)
            energy = self.get_energy(pts_feat, rgb_feat, inp_variable_sampled_pose, t, decoupled_rt=False)
