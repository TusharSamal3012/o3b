...
import torch
from o3b.cv.geometry.transform import reproj2d3d_broadcast
from o3b.cv.geometry.transform import rot3x3
from o3b.cv.geometry.transform import so3_exp_map
...
def axis3d_from_pxl2d(kpts2d_orient, cam_intr4x4):
    ...
    for key in kpts2d_orient.keys():
        ...
        if len(kpts2d_orient_lr) > 0:
            kpts3d_homog_orient_lr = reproj2d3d_broadcast(
                kpts2d_orient_lr,
                cam_intr4x4.inverse(),
            )

            vector_homog_orient_lr = (
                kpts3d_homog_orient_lr[:, 0] - kpts3d_homog_orient_lr[:, 1]
            )
            vector_homog_orient_lr = (
                vector_homog_orient_lr
                / vector_homog_orient_lr.norm(dim=-1, keepdim=True)
            )
            ...
            for i in range(len(kpts3d_homog_orient_lr)):
                orient3d_plane_i = torch.linalg.cross(
                    kpts3d_homog_orient_lr[i, 0],
                    kpts3d_homog_orient_lr[i, 1],
                )
                orient3d_plane_i = orient3d_plane_i / orient3d_plane_i.norm(
                    dim=-1,
                    keepdim=True,
                )
                vector_homog_orient_i = (
                    kpts3d_homog_orient_lr[i, 0] - kpts3d_homog_orient_lr[i, 1]
                )
                vector_homog_orient_i = (
                    vector_homog_orient_i
                    / vector_homog_orient_i.norm(dim=-1, keepdim=True)
                )
            ...
            if len(orient3d_planes) > 1:
                ...
                if (eins < 0.0).all():
                    ...
                    logger.warning(
                        f"Inconsistency of axis3d with axis2d alignment. Skipping orient for {key}...",
                    )
                    orients[key] = []
            else:
                orients[key] = []
    ...
    orients = torch.stack(
        [orients["left-right"], orients["back-front"], orients["top-bottom"]],
        dim=0,
    )

    from o3b.cv.geometry.transform import so3_log_map
    ...
    l_aligned = torch.tensor(
        torch.zeros_like(b_aligned),
        requires_grad=True,
        dtype=torch.float64,
    )
    ...
    for i in range(500):
        ...
        optimizer = torch.optim.SGD(
            params=[cam_rot3x3_obj],
            lr=0.1,
        )  # 0.01 too low -> no update, too large oscillating

        loss_orthog_kpts = (
            torch.bmm(A_orthog[None,], cam_rot3x3_obj.T.reshape(1, 9, 1))[0, :, 0]
        ).norm()
        loss_orthog_coords = (
            (
                rot3x3(cam_rot3x3_obj.T, cam_rot3x3_obj)
                - torch.eye(3, device=cam_rot3x3_obj.device)
            )
            .flatten()
            .norm()
        )  # ().mean()
    ...
    logger.info(
        f"Alignment {torch.bmm(A_aligned[None,], cam_rot3x3_obj.T.reshape(1, 9, 1))[0, :, 0]}",
    )
