...
def fit_sphere_to_pts3d(
    ...
    Returns:
        spheres (Spheres): single mesh with the cuboid
        spheres_tform4x4_obj (torch.Tensor): transformation from object coordinate system to cuboid
    """
    ...
    tmp_tform6_sphere = torch.zeros(6).to(dtype=dtype, device=device)

    if optimize_transl:
        pts3d_mean = (sphere_pts3d_limits[3:6] + sphere_pts3d_limits[0:3]) / 2.0
        tmp_tform6_sphere[:3] = -pts3d_mean

    spheres_tform4x4_obj = se3_exp_map(tmp_tform6_sphere)

    from o3b.cv.geometry.objects3d.meshes import Meshes

    sphere_size = (sphere_pts3d_limits[3:6] - sphere_pts3d_limits[0:3]).max()

    if size is not None:
        scale = size / sphere_size
        spheres_tform4x4_obj[:3] *= scale
        sphere_size *= scale
