...
from o3b.cv.io import write_pts3d_with_colors_and_normals
import re
import numpy as np
...
def sorted_image_list(image_list):
    return sorted(
        image_list,
        key=lambda f: [atoi(val) for val in re.split(r"(\d+)", Path(f).stem)],
    )
...
def run_droid_slam(
    ...
    run_cmd(
        cmd=f'echo "{fx} {fy} {cx} {cy}" > {path_out_root}/{rpath_out}/calib.txt',
        logger=logger,
    )
    run_cmd(
    ...
    partial_tstamps = torch.load(path_out.joinpath("tstamps.pt")).to(
        dtype=torch.long,
        device=device,
    )
    partial_intr4 = torch.load(path_out.joinpath("intr.pt")).to(
        dtype=torch.long,
        device=device,
    )[0]
    ...
    partial_cams_intr4x4 = torch.stack(
        [partial_cams_intr4x4.clone() for _ in range(len(partial_tstamps))],
        dim=0,
    ).to(device=device)

    partial_disps_up = torch.load(path_out.joinpath("disps.pt"))[:, None].to(
        device=device,
    )
    partial_masks_up = torch.load(path_out.joinpath("masks.pt"))[:, None].to(
        device=device,
    )
    ...
    partial_depths = 1.0 / partial_disps_up
    partial_depths = partial_depths.nan_to_num(0.0, posinf=0.0, neginf=0.0).to(
        torch.float,
    )

    rgbs = torch.stack([read_image(fpath_rgb) for fpath_rgb in fpaths_rgbs], dim=0).to(
        device=device,
    )
    ...
    partial_depths_valid = (
        (
            partial_disps_up
            > partial_disps_up.flatten(2).mean(dim=-1)[
                ...,
                None,
                None,
            ]
        )
        * partial_depths.isfinite()
        * (partial_depths > 0.0)
        * partial_masks_up
    )
    partial_depths[~partial_depths_valid] = 0.0
    ...
    pts3d = transf3d_broadcast(
        depth2pts3d_grid(partial_depths[::spts], partial_cams_intr4x4[::spts]).permute(
            0,
            2,
            3,
            1,
    ...
    pts3d_colors = (
        partial_rgb[::spts].permute(0, 2, 3, 1)[
            partial_depths_valid[::spts][..., 0, :, :]
        ]
        / 255.0
    )
    ...
    if (pts3d_normals_obj == 0.0).all():
        o3d_pcl.estimate_normals()
        pts3d_normals_obj = torch.from_numpy(np.array(o3d_pcl.normals)).to(
            device=pts3d_normals_obj.device,
            dtype=pts3d_normals_obj.dtype,
        )
    else:
        o3d_pcl.normals = open3d.utility.Vector3dVector(
            pts3d_normals_obj.detach().cpu().numpy(),
        )  # invalidate existing normals
    o3d_pcl.colors = open3d.utility.Vector3dVector(
        pts3d_colors_obj.detach().cpu().numpy(),
    )
    # open3d.visualization.draw_geometries([o3d_pcl])
    ## DEBUG BLOCK END
    ...
    cams_tform4x4_obj_centered = tform4x4_broadcast(
        cams_tform4x4_obj,
        inv_tform4x4(center3d_tform4x4_obj),
    )
    # save transformation
    ...
    from o3b.cv.geometry.fit.rays_center3d import fit_rays_center3d
