...
def getProjectionMatrixFromIntrinsics(znear, zfar, fx, fy, W, H, cx, cy):
    ...
    P[0, 0] = 2 * fx / W
    P[1, 1] = 2 * fy / H
    P[0, 2] = 2 * (cx / W) - 1.0  # or -1.
    P[1, 2] = 2 * (cy / H) - 1.0  # or -1.
...
def render_gaussians(
    ...
    pts3d_mask = (
        pts3d_mask & (pts3d[:, :, 2] >= z_near) & (pts3d[:, :, 2] <= z_far)
    )  # otherwise illegal access memory
    ...
    for b in range(B):
        ...
        fullprojmatrix = (viewmatrix.unsqueeze(0).bmm(projmatrix.unsqueeze(0))).squeeze(
            0,
        )
        ...
        means2d_b = (
            torch.zeros_like(pts3d_b) + 0
        )  # torch.zeros((N, 2)).to(device) # N x 2
        ...
        for f in range(feats_splits):
            ...
            feats_b_f[:, :F_f] = feats_b[
                :,
                f * feats_dim_base : f * feats_dim_base + F_f,
            ]
            ...
            shs = None
            scales = (
                pts3d_size_b  #  torch.ones((N, 3)).to(device) * pts3d_size  # N x 3
            )
