...
import od3d_basic.cv.metric.pt3d_so3  # import so3_exp_map, so3_log_map
from od3d_basic.cv.metric.pt3d_rotation_conversions import (
...
def so3_exp_map(so3_log: torch.Tensor):
    ...
    so3_3x3 = od3d_basic.cv.metric.pt3d_so3.so3_exp_map(so3_log.reshape(-1, 3))
...
def so3_log_map(so3_3x3: torch.Tensor):
    ...
    so3_log = od3d_basic.cv.metric.pt3d_so3.so3_log_map(so3_3x3.reshape(-1, 3, 3))
...
def get_cam_front_back_top_tform4x4_obj(device=None, dist=0.):
    from od3d_basic.cv.visual.show import CAM_TFORM_OBJ
...
# from od3d_basic.cv.geometry.transform import transf4x4_from_spherical, inv_tform4x4
# import torch
...
def get_ico_cam_tform4x4_obj_for_viewpoints_count(
    ...
    return cam_tform4x4_obj
    # look_at_rotation()
    # from od3d_basic.cv.visual.show import show_scene
...
from od3d_basic.cv.differentiation.gradient import calc_batch_gradients
